...
```mermaid
flowchart TD
    A[Мобильные приложения] --> B{Платформа}
    
    B -->|iOS| C[Регистрация в APNS<br/>Получение device token]
    B -->|Android| D[Регистрация в FCM<br/>Получение device token]
    
    C --> E[Сервис управления устройствами]
    D --> E
    
    E --> F[Событийная шина<br/>Kafka/RabbitMQ]
    
    G[Сервис корзины] --> F
    H[Сервис заказов] --> F
    I[Сервис маркетинга] --> F
    
    F --> J[Сервис отправки PUSH-уведомлений]
    
    J --> K{FCM/APNS}
    K -->|Android| L[FCM Google]
    K -->|iOS| M[APNS Apple]
    
    L --> N[Android устройства]
    M --> O[iOS устройства]
